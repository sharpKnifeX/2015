...
public:
	...
	enum Formats {						//K线的复权状态
		formatNone		=	0x00,
		formatMin		=	0x01,
		formatOriginal	=	0x01,		//不复权
		formatXDRup		=	0x02,		//向上复权（后复权）
		formatXDRdown	=	0x03,		//向下复权（前复权）
		formatMax		=	0x03,
	};

	enum MaindataTypes {				//主数据类型，当需要在几个数据中取一个作为主数据时，取哪个
		mdtypeNone		=	0x00,
		mdtypeMin		=	0x01,
		mdtypeClose		=	0x01,
		mdtypeOpen		=	0x02,
...
protected:
	int		m_nKType;			//K线类型，记录这个数组是日线、周线或5分钟线等，见CKData::KTypes枚举类型。
	int		m_nCurFormat;		//K线的复权状态，见CKData::Formats枚举类型。
	int		m_nCurMaindataType;	//主数据类型，见CKData::MaindataTypes枚举类型。例如：如果是mdtypeAverage，则MaindataAt(...)函数返回的就是平均价。
	CDRData	m_drdata;
